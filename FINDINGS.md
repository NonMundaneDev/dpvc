...
The failure-conditioned selector then filtered the current reference condition
for clean positive style targets: `emotion_miss + style_to_neutral`, excluding
high WER, low MOS, low novelty, content collapse, identity collapse, and mixed
collapse:
...
9. **The first failure-conditioned target selector narrows the next training
   run to anger/disgust.** Under the current reference guard, `anger` has `5`
   and `disgust` has `6` clean neutral-collapse targets, while `fear` has
   `0/11` clean targets after content/naturalness/novelty/collapse exclusions.
   Fear remains important, but it should not be used as a positive target until
   its content/naturalness confound is separated. Finding 33 tests that
   `anger`/`disgust` follow-up directly.
...
content. The first selector applies that rule and recommends an
`anger`/`disgust` target-dim teacher follow-up, leaving `fear` as a blocked
diagnostic style for now. Finding 33 shows that this clean target selection is
necessary but not sufficient: target-dim teacher pressure still needs an
explicit anti-neutral or generated-audio-calibrated output signal.
...
## Finding 33: Failure-Conditioned Target-Dim Supervision Is Not Enough to Escape Neutral Collapse

**Branch:** `research/controllable-vae`

**Question.** Once generated-audio failure mining identifies clean
`anger`/`disgust` rows that miss the target emotion by collapsing to neutral,
can a conservative target-dim style-teacher objective repair those styles
without sacrificing the expanded rare-supply guard's WER/MOS tradeoff?

### Setup

Checkpoint trained:

- `embeddings/openvoice_vae_mixed_teacher_cvrare_failure_targeted_style_teacher_labeled_warmup.pt`
...
- Style-teacher ramp: `0.0 -> 0.25`
- Target mode: `target_dim`
- Required labels: enabled
- Positive style weights:
  `anger=3`, `disgust=3`, all other styles `0`
- Teacher-style rows: `129/14195`
...
|-----------|--------|--------------|-----------------|-----------|------------------|---------------------------|-------------------|----------------|-------------------------|----------|
| `mixed_teacher_cvrare_hybrid_style_distill_labeled_warmup_sad_enunc_guard` | `47.0%` | `0.2726` | `0.2348` | `-0.2081` | `2` | `18` | `1` | `1` | `20` | Current quality-balanced reference |
| `mixed_teacher_cvrare_failure_targeted_style_teacher_labeled_warmup` | `39.4%` | `0.2960` | `0.2651` | `-0.2072` | `1` | `26` | `1` | `0` | `28` | Better content-collapse count, but worse recall and neutral collapse |
| `mixed_teacher_cvrare_decoder_proto_labeled_warmup` | `42.4%` | `0.3008` | `0.2863` | `-0.2148` | `4` | `22` | `1` | `0` | `27` | Decoder-prototype cautionary baseline |
| `mixed_teacher_cvrare_decoder_proto_w005_labeled_warmup` | `42.4%` | `0.3032` | `0.2782` | `-0.2122` | `3` | `22` | `1` | `0` | `26` | Lower-weight prototype still misses the reference |

Per-style recall for the failure-targeted follow-up:
...
Failure mining after adding the targeted condition:
...
1. **The selector found real failures, but the loss was too indirect.**
   `anger` improved relative to the current guard (`2/11` vs `1/11`), but
   `disgust` fell to `0/11`, and overall recall dropped from `47.0%` to
   `39.4%`.
2. **The targeted objective mainly reduced content collapse, not style
   collapse.** Content collapse improved from `2` to `1`, but
   style-to-neutral collapse worsened from `18` to `26`.
3. **The current `sad/enunciated` guard remains the quality-balanced
   reference.** The targeted run nearly ties MOS delta (`-0.2072` vs
   `-0.2081`) but loses on recall, WER, and collapse.
4. **Clean positive rows are not enough if the decoded audio can still sound
   neutral.** The next objective needs an anti-neutral or generated-audio
   calibrated signal that penalizes observed neutral-classified output, rather
   than only pulling latent target dimensions.

### Implication

Finding 33 narrows the research gap again. The problem is no longer just
"which rows should we target?" The selector answered that. The new problem is
"what training signal forces the decoder output to leave the neutral basin
while preserving WER/MOS?" The next best branch should test an explicit
anti-neutral / generated-audio-calibrated objective, and it should keep the
current `sad/enunciated` guard as the reference until a training-side method
beats it.
...
## Finding 38: The Content-Repair Gate Blocks All Current Hard-Style Strength Candidates

### Setup

Finding 35 showed that generated-audio strength-grid reranking can improve
classifier recall on individual hard-style rows, especially `anger_s10` and
`fear_s7p5`, but Joe's first listening review did not hear a perceptual
candidate win.

Finding 38 turns that judgment into a reproducible gate:
...
1. target-label gain over the current `sad/enunciated` guard;
2. content safety: candidate WER no more than `0.30` and WER increase no more
   than `0.15`;
3. naturalness safety: candidate MOS delta no worse than `-0.75` and MOS
   delta drop no worse than `0.25`;
4. novelty safety: novelty delta no worse than `-0.02`;
5. perceptual gate: if human ratings exist, the candidate must be preferred.
...
Style-level decisions:

| Style | Rows | Objective-pass rows | Perceptually blocked | Promoted | Decision |
|-------|------|---------------------|----------------------|----------|----------|
| `anger` | `11` | `1` | `1` | `0` | diagnostic only |
| `disgust` | `11` | `0` | `0` | `0` | needs content repair |
| `fear` | `11` | `1` | `1` | `0` | diagnostic only |

The two objective-pass rows were:

| Style | Source | Candidate | Objective result | Human result |
|-------|--------|-----------|------------------|--------------|
| `anger` | `cremad_1006` | `anger_s10` | passes content/style gate | tie / indistinguishable |
| `fear` | `male_1_cremad_1003` | `fear_s7p5` | passes content/style gate | reference preferred; candidate pitch change sounded unnatural |

### Implication

This finding is a guardrail for the paper and demo:

1. **No hard-style strength preset should be promoted yet.** The current grid
   can identify classifier gains, but none pass both objective and perceptual
   gates.
2. **`disgust` is the clearest repair target.** It has no objective-pass row
   in the current grid, so stronger inference strength is not enough.
3. **`fear` needs content/naturalness diagnosis before more recall chasing.**
   The best target-gain row was perceptually worse because of an unnatural
   pitch change.
4. **The next real repair must optimize generated audio, not just embeddings or
   classifier labels.** The strength grid is diagnostic evidence for a future
   generated-audio-calibrated objective, not a solved control policy.
...
## Finding 39: Generated-Audio-Calibrated Training Does Not Beat the Current Guard

### Setup

Finding 38 showed that the hard-style strength grid should stay diagnostic:
objective metrics found a few better `anger` / `fear` rows, but Joe's
perceptual review found no candidate wins and `disgust` had no objective-pass
repair row.

Finding 39 tests the next training-side version of that idea. The
generated-audio objective plan selected:

- `anger` for conservative repair pressure;
- `disgust` because it had no safe inference-side repair row;
- `fear` blocked because its best metric row sounded unnaturally pitch-shifted
  and the clean-target selector still had no safe reference rows.

The resulting checkpoint is:

- `embeddings/openvoice_vae_mixed_teacher_cvrare_audio_calibrated_labeled_warmup.pt`

It was evaluated on the same 11-source / 10-style OpenVoice panel used by the
current expanded rare-supply comparisons.
...
Per-style recall for the targeted hard styles:
...
| Listener | Style | Human-perceptual read |
|----------|-------|-----------------------|
| Stephen | `disgust` | Sounds convincingly disgusted and remains intelligible |
| Stephen | `anger` | Some style change is audible, but the speech is distorted and less intelligible |
| Joe | `disgust` | Sounds neutral for all focused rows |
| Joe | `anger` | Early rows up to `cremad_1076` sound slightly more angry; later rows sound more neutral |
...
2. **Speaker movement is easier than target-style control.** Both native
   ...
   region.
3. **`disgust` is not confirmed as a perceptual success.** It stayed at
   `0/11` emotion2vec recall even when the objective explicitly targeted it,
   and Joe heard the focused `disgust` rows as neutral. Stephen's first
   positive read should be treated as a listener hypothesis that did not
   survive Joe's review.
...
5. **Training-label perceptual strength is now a central limitation.** Joe went
   back to CREMA-D training examples and found that many `disgust` examples also
   sound neutral to him. If the source label is subtle or ambiguous, more
   objective pressure is unlikely to create a robust, generalizable perceptual
   control.

The next repair should not be another latent-only or prototype-weight tweak.
It should use generated-audio behavior and training-data label quality more
directly: audit which labels are perceptually strong enough to support the
paper/demo, focus on controls with clear signal, and treat subtle hard emotions
such as `disgust` as limitations unless stronger training examples are added.
...
## Finding 41: Control Shortlist Narrows the Paper Claims Before More Training

**Question.** Given the source separability audit, generated-output metrics, and
Joe/Stephen listening evidence, which controls should be paper/demo headline
candidates now?

**Method.** We built `scripts/build_control_selection_recommendation.py` to
merge:

- source-label separability from `results/training_style_separability_by_label.csv`
- current reference-guard generated emotion/WER/MOS metrics
- ECAPA external speaker novelty for the current reference guard
- collapse diagnostics for `mixed_teacher_cvrare_hybrid_style_distill_labeled_warmup_sad_enunc_guard`
- structured perceptual evidence from `results/control_selection_perceptual_evidence.csv`

The rule is intentionally conservative: source separability decides which
controls are fair to evaluate, generated metrics decide whether the current
output is plausible, and positive focused listening evidence is required before
a control becomes a headline claim.

**Result.**

| Bucket | Styles | Meaning |
| --- | --- | --- |
| candidate headline, pending listening | `neutral`, `sad` | strong source and generated metrics, but no focused listening confirmation recorded |
| supported but quality-sensitive | `enunciated`, `happy`, `whisper` | plausible secondary/demo controls with quality, source-strength, or metric caveats |
| diagnostic / limitation | `anger`, `confused`, `disgust`, `fear` | do not use as headline claims under current evidence |

Notable rows:

- `neutral`: source separable, generated strong (`0.9091` recall, `0.1269`
  WER, `-0.1472` MOS delta), but still needs focused listening.
- `sad`: source separable, generated strong (`0.9091` recall, `0.2955` WER,
  `-0.0318` MOS delta), but still needs focused listening.
- `happy`: source separable, but generated evidence is mixed (`0.4545` recall,
  `0.3981` WER), so it is not a headline claim yet.
- `whisper`: generated non-emotion metrics look promising (`0.2889` WER,
  `-0.0668` MOS delta, `0.4434` external novelty), but the source audit marks
  it quality-sensitive rather than strongly separable, so it stays secondary.
- `disgust`: source separable, but generated recall is weak (`0.1818`) and Joe
  heard focused generated `disgust` as neutral, so it stays diagnostic.

**Implication for the paper.** The paper should not claim "all nine controls
work." The current defensible story is narrower: identity shift is strong, the
current reference guard is the best overall system, `neutral`/`sad` are the
first style controls to confirm perceptually, `happy`/`whisper`/`enunciated`
are secondary candidates, and hard emotions remain limitations unless a later
listening result changes the gate.
