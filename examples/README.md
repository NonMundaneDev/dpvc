...
Agreement-rule follow-up on the same teacher:

```bash
python scripts/annotate_commonvoice_pseudolabels.py \
    --embeddings embeddings/openvoice_commonvoice_cv500_emb.pt \
    --output embeddings/openvoice_commonvoice_cv500_pseudo_agreement_scored.pt \
    --top-k 3 \
    --save-style-score-map \
    --consistency-view center_crop \
    --consistency-crop-frac 0.8 \
    --consistency-min-seconds 1.0

python scripts/filter_commonvoice_pseudolabels.py \
    --input embeddings/openvoice_commonvoice_cv500_pseudo_agreement_scored.pt \
    --output embeddings/openvoice_commonvoice_cv500_pseudo_agreement_mapped015_filtered.pt \
    --acceptance-policy balanced_targets \
    --style-thresholds neutral=0.995,sad=0.98,happy=0.92,disgust=0.92,anger=0.90,fear=0.90 \
    --style-targets neutral=120,sad=110,happy=60,disgust=50,anger=20,fear=20 \
    --require-secondary-agreement \
    --secondary-agreement-mode mapped_score \
    --secondary-style-thresholds anger=0.15,disgust=0.15,fear=0.15,happy=0.15,neutral=0.15,sad=0.15

python scripts/build_mixed_training_set.py \
    --commonvoice embeddings/openvoice_commonvoice_cv500_pseudo_agreement_mapped015_filtered.pt \
    --cremad embeddings/openvoice_cremad_emb.pt \
    --expresso embeddings/openvoice_expresso_emb.pt \
    --output embeddings/openvoice_mixed_teacher_mapped015_base.pt \
    --commonvoice-max-speakers 500 \
    --commonvoice-max-clips-per-speaker 1 \
    --acceptance-policy artifact_selected \
    --commonvoice-prefer-pseudo
...
python examples/openvoice_train_vae_mixed.py \
    --embeddings embeddings/openvoice_mixed_teacher_mapped015_base.pt \
    --output embeddings/openvoice_vae_mixed_teacher_mapped015_balanced.pt \
    --schedule static_balanced

python scripts/run_ablation_inference.py \
    --source-dir examples/source_speakers/ \
    --condition mixed_teacher_mapped015_balanced \
    --out output/mixed_teacher_mapped015_balanced_eval \
    --style-strength 5.0 \
    --noise-level 0.0 \
    --seed 42

python examples/eval_emotion.py --input output/mixed_teacher_mapped015_balanced_eval --out results/eval_emotion_mixed_teacher_mixed_teacher_mapped015_balanced.csv
python examples/eval_novelty.py --manifest output/mixed_teacher_mapped015_balanced_eval/generation_manifest.jsonl --out results/eval_novelty_mixed_teacher_mixed_teacher_mapped015_balanced.csv
python examples/eval_wer.py     --input output/mixed_teacher_mapped015_balanced_eval --out results/eval_wer_mixed_teacher_mixed_teacher_mapped015_balanced.csv
python examples/eval_mos.py     --input output/mixed_teacher_mapped015_balanced_eval --out results/eval_mos_mixed_teacher_mixed_teacher_mapped015_balanced.csv
```

Latent-prototype alternate-teacher follow-up:

```bash
python scripts/annotate_commonvoice_latent_prototypes.py \
    --commonvoice embeddings/openvoice_commonvoice_cv500_emb.pt \
    --combined embeddings/openvoice_combined_emb.pt \
    --checkpoint embeddings/openvoice_vae_combined.pt \
    --output embeddings/openvoice_commonvoice_cv500_pseudo_prototype.pt \
    --temperature 0.08 \
    --report-threshold 0.35

python scripts/filter_commonvoice_pseudolabels.py \
    --input embeddings/openvoice_commonvoice_cv500_pseudo_prototype.pt \
    --output embeddings/openvoice_commonvoice_cv500_pseudo_prototype_filtered.pt \
    --acceptance-policy balanced_targets \
    --default-threshold 0.35 \
    --style-targets anger=40,confused=40,disgust=40,enunciated=40,fear=40,happy=40,neutral=40,sad=40,whisper=40

python scripts/build_mixed_training_set.py \
    --commonvoice embeddings/openvoice_commonvoice_cv500_pseudo_prototype_filtered.pt \
    --cremad embeddings/openvoice_cremad_emb.pt \
    --expresso embeddings/openvoice_expresso_emb.pt \
    --output embeddings/openvoice_mixed_teacher_prototype_base.pt \
    --commonvoice-max-speakers 500 \
    --commonvoice-max-clips-per-speaker 1 \
    --acceptance-policy artifact_selected \
    --commonvoice-prefer-pseudo
...
python examples/openvoice_train_vae_mixed.py \
    --embeddings embeddings/openvoice_mixed_teacher_prototype_base.pt \
    --output embeddings/openvoice_vae_mixed_teacher_prototype_balanced.pt \
    --schedule static_balanced

python scripts/run_ablation_inference.py \
    --source-dir examples/source_speakers/ \
    --condition mixed_teacher_prototype_balanced \
    --out output/mixed_teacher_prototype_balanced_eval \
    --style-strength 5.0 \
    --noise-level 0.0 \
    --seed 42

python examples/eval_emotion.py --input output/mixed_teacher_prototype_balanced_eval --out results/eval_emotion_mixed_teacher_mixed_teacher_prototype_balanced.csv
python examples/eval_novelty.py --manifest output/mixed_teacher_prototype_balanced_eval/generation_manifest.jsonl --out results/eval_novelty_mixed_teacher_mixed_teacher_prototype_balanced.csv
python examples/eval_wer.py     --input output/mixed_teacher_prototype_balanced_eval --out results/eval_wer_mixed_teacher_mixed_teacher_prototype_balanced.csv
python examples/eval_mos.py     --input output/mixed_teacher_prototype_balanced_eval --out results/eval_mos_mixed_teacher_mixed_teacher_prototype_balanced.csv

python scripts/build_mixed_training_set.py \
    --commonvoice embeddings/openvoice_commonvoice_cv500_pseudo_prototype_filtered.pt \
    --cremad embeddings/openvoice_cremad_emb.pt \
    --expresso embeddings/openvoice_expresso_emb.pt \
    --output embeddings/openvoice_mixed_teacher_prototype_guarded_base.pt \
    --commonvoice-max-speakers 500 \
    --commonvoice-max-clips-per-speaker 1 \
    --acceptance-policy artifact_selected \
    --commonvoice-prefer-pseudo \
...
python examples/openvoice_train_vae_mixed.py \
    --embeddings embeddings/openvoice_mixed_teacher_prototype_guarded_base.pt \
    --output embeddings/openvoice_vae_mixed_teacher_prototype_guarded.pt \
    --schedule labeled_finish \
    --schedule-epochs 1000 \
    --schedule-end-masses CommonVoice=0.10,CREMA-D=0.45,Expresso=0.45
...
python scripts/run_ablation_inference.py \
    --source-dir examples/source_speakers/ \
    --condition mixed_teacher_prototype_guarded \
    --out output/mixed_teacher_prototype_guarded_eval \
    --style-strength 5.0 \
    --noise-level 0.0 \
    --seed 42

python examples/eval_emotion.py --input output/mixed_teacher_prototype_guarded_eval --out results/eval_emotion_mixed_teacher_mixed_teacher_prototype_guarded.csv
python examples/eval_novelty.py --manifest output/mixed_teacher_prototype_guarded_eval/generation_manifest.jsonl --out results/eval_novelty_mixed_teacher_mixed_teacher_prototype_guarded.csv
...
python examples/openvoice_train_vae_mixed.py \
    --embeddings embeddings/openvoice_mixed_teacher_hybrid_extra_base.pt \
    --output embeddings/openvoice_vae_mixed_teacher_hybrid_style_distill_balanced.pt \
    --schedule static_balanced \
    --style-teacher-checkpoint embeddings/openvoice_vae_combined.pt \
...
python scripts/run_ablation_inference.py \
    --source-dir examples/source_speakers/ \
    --condition mixed_teacher_hybrid_style_distill_balanced \
    --out output/mixed_teacher_hybrid_style_distill_balanced_eval \
    --style-strength 5.0 \
    --noise-level 0.0 \
    --seed 42

python examples/eval_emotion.py --input output/mixed_teacher_hybrid_style_distill_balanced_eval --out results/eval_emotion_mixed_teacher_mixed_teacher_hybrid_style_distill_balanced.csv
python examples/eval_novelty.py --manifest output/mixed_teacher_hybrid_style_distill_balanced_eval/generation_manifest.jsonl --out results/eval_novelty_mixed_teacher_mixed_teacher_hybrid_style_distill_balanced.csv
...
- `mixed_teacher_threshold_balanced` is still the best overall result from this branch, because it matches `mixed_quality_labeled_guarded` on recall while improving novelty, WER, MOS, and identity collapse
